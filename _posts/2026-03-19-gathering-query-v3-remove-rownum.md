---
title: "모임 조회 3단계 — ROW_NUMBER 제거로 5초 → 317ms"
...
tags:
  ...
  - Locust
date: 2026-03-19
excerpt: "ROW_NUMBER()를 제거하고 카테고리별 단순 쿼리로 전환하여, 복합 인덱스가 비로소 동작하게 만든 과정."
---
...
> 3단계. ROW_NUMBER 제거 — 5초 → 317ms *(현재 글)*
...
2단계에서 쿼리 분리로 389초 → 5초까지 개선했지만, **ROW_NUMBER()가 풀 스캔을 강제하는 구조적 한계**가 남아 있었다.
...
`(category_id, count)` 복합 인덱스가 존재하지만, ROW_NUMBER()는 **전체 행에 번호를 매긴 뒤** `WHERE rownum BETWEEN 1 AND 9`로 필터링하므로, MySQL 옵티마이저는 인덱스 대신 풀 스캔을 선택할 수밖에 없다.
...
- `Index lookup on g using idx_gathering_category_count` — rows=9, actual time **0.02~0.05ms**
- `Limit: 9 row(s)` — 인덱스에서 9건 읽고 즉시 반환
- 개별 쿼리: **0.17ms**
...
이전 61% 실패에서 **0% 실패**로 전환됐다.

### Grafana — HikariCP

![v3 Grafana HikariCP](/assets/images/gathering/세번째%20그라파나.png)
...
![v3 DBLoadCPU](/assets/images/gathering/세번째%20db.png)
...
다만 **p50 1,600ms / p95 1,900ms**로 응답 지연이 남아 있다. 카테고리 수만큼 쿼리가 발생하는 구조적 한계 — 카테고리가 늘어날수록 쿼리 수가 선형 증가한다. 이 한계를 넘으려면 **캐시 레이어**가 필요하다.
